...
public class ToMapperAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        String mapperName = className.replace(".java", ".xml");

        Project project = e.getProject();
        PsiFile[] files = FilenameIndex.getFilesByName(project, mapperName, GlobalSearchScope.allScope(project));
        if (files.length == 0) {
            PsiFile[] psiFile = PsiShortNamesCache.getInstance(project).getFilesByName(mapperName);
        ...
        if (files.length == 1) {
            ...
            if (StringUtil.isNotEmpty(xml) && xml.contains("id=\"" + methodName + "\"")) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    ...
                    BalloonBuilder builder = factory.createHtmlTextBalloonBuilder("no such method!", null,
                            new JBColor(new Color(39, 49, 39), new Color(39, 64, 39)), null);
    ...
    private void toMapper(Project project, String methodName, VirtualFile mapperFile, String xml) {
        ...
        //获取sql所在的行数，这里用了比较笨的方法。api找了很久没找到有什么方法可以获取行号
